...
const isimler = require("../../../database/isimler.js");
  ...
  .addStringOption(option =>
    option.
      ...
      .setRequired(true)
            ...
            const Veri = await isimler.findOne({ Oyuncu: sebep });
            if (!Veri) return interaction.reply({ content: "<@" + sebep + "> kişisinin rol bilgisi veritabanında bulunmadı."}).catch(() => {});
            let page = 1;
            let rol = Veri.İsimler.sort((a, b) => b.Tarih - a.Tarih)
            let liste = rol.map((x , index) => `\`${index + 1}.\` - <@${x.Yetkili}> » \`${x.Eİsim}\` » \`${x.Yİsim}\` \n \`${x.Tarih}\``)
