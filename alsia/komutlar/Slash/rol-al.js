...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("rolal")
    .setDescription("Bir oyuncunun üzerinde olan bir permi almaya işe yarar.")	
...
setName('rol')
.setDescription('Alınıcın rolü lütfen seçiniz Örnek: LSPD')
.setRequired(true)),
			...
			alsia: async (client, interaction) => {
                ...
                if (oyuncu.roles.cache.has(rol)) return interaction.reply({ embeds: [Warn.setDescription(`<a:unlemsel:1327600285597569066> ・ ***Uyarı:*** *Bu rol zaten oyuncuda yok.*`)] , ephemeral: true  })
                ...
                await (oyuncu.roles.remove(rol)).catch(() => {});

              //  await interaction.deferReply({  }).catch(() => {});

                const embed = new EmbedBuilder()
                .setColor('#041f49')
                .setDescription(`<a:utility:1327600287367696515> ・ ${oyuncu} *isimli oyuncudan* ${rol} *isimli rol başarılı bir şekilde alındı.*
               ...
               <a:utility:1327600287367696515> ・ \`ᴀʟıɴᴀɴ ʀᴏʟ:\` ${rol}
