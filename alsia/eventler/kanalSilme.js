...
module.exports = {
	...
	başlat: async(channel) => {
        ...
        if (channel.guild.id !== ayarlar.Bot.SunucuID) return
        ...
        const kanalLOG = channel.guild.channels.cache.get(ayarlar.LOG.SkanalLOG);
        if (!kanalLOG) return;
      ...
      .setThumbnail(executor.avatarURL({dynamic:true})) 
        .setDescription(`<a:unlemsel:1327600285597569066> ・ *Bir yetkili tarafından* | \`${channel.name} | ${channel.id}\` | *isimli kanal silindi.* 
                        
        <:king_crown:1327600238407450697> ・ \`ʏᴇᴛᴋɪʟɪ:\` ${executor}
        <a:5961darkbluetea:1327585257578561548> ・ \`ᴋᴀɴᴀʟ ᴛᴜʀᴜ: ${tür[channel.type]}\``)
        .setColor("#12073d")
        .setFooter({ text: moment(Date.now()).format("LLL") })
      
      
        channel.guild.channels.cache.get(ayarlar.LOG.SkanalLOG).send({ embeds: [embed] })
